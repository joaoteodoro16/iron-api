    ...
    IValidator<CreateUserRequest> validator)
{
    ...
    public async Task<Result<long>> ExecuteAsync(CreateUserRequest request)
    {
        ...
        if (!validationResult.IsValid)
            return Result.Fail<long>(validationResult.Errors.Select(e => e.ErrorMessage).ToList());

        var emailResult = Result.Try(() => Email.Create(request.Email));
        if (emailResult.IsFailure)
            return Result.Fail<long>(emailResult.Error);

        var phoneResult = Result.Try(() => PhoneNumber.Create(request.PhoneNumber));
        if (phoneResult.IsFailure)
            return Result.Fail<long>(phoneResult.Error);

        var emailInUse = await _userRepository.ExistsByEmailAsync(emailResult.Value);
        if (emailInUse)
            return Result.Fail<long>("Já existe um usuário cadastrado com esse email.");
        ...
        if (userResult.IsFailure)
            return Result.Fail<long>(userResult.Error);
